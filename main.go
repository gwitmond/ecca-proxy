...
func eccaProxy (r *http.Request, ctx *goproxy.ProxyCtx) (*http.Request, *http.Response) {
	log.Println("\n\n\nRequest is ", r.Method, r.URL.String())
	if r.URL.Scheme == "http" {
		r.URL.Scheme = "https"
	}
	
	resp, err := fetchRequest(r)
	if err != nil {
	...
	// On redirect change the response location from https to http. 
	// So the client comes back to us, he doesn't have the keys we have.
...
func fetchRequest(req *http.Request) (*http.Response, error) {
	// clean up the recycled header we got from the user
	req.RequestURI="" 

	var caCert *x509.Certificate
	serverCred, ok := getServerCreds(req.URL.Host)
	if ok == true {
	...
	// just to make sure that we don't leave previously used certificates when a user changes accounts
	...
	setServerCreds(req.URL.Host, serverCert{
		// realm: auth["realm"], 
